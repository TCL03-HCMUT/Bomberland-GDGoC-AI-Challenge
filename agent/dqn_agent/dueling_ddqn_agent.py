...
class DuelingDQNModel(nn.Module):
    ...
    Dueling DQN Architecture:
      - Shared Feature Extractor (Conv2D + MLP)
      - Value Stream V(s): How good is the state?
      - Advantage Stream A(s, a): How good is the action compared to others?
    """
    def __init__(self, map_shape, aux_dim, output_dim):
        super().__init__()
        c, h, w = map_shape
        
        # 1. Shared Spatial Map Encoder
        ...
        # 2. Shared Auxiliary Scalar Encoder
        ...
        shared_dim = conv_out_dim + 32

        # 3a. Value Stream: V(s) -> Outputs a single scalar
        ...
        # 3b. Advantage Stream: A(s, a) -> Outputs a scalar for each action
    ...
    def forward(self, map_x, aux_x):
        ...
        shared_feat = torch.cat([map_feat, aux_feat], dim=1)
        ...
        # The mean subtraction ensures that the network is identifiable (prevents the
        # value and advantage streams from adding constants to each other)
...
class DuelingDDQNTrainingAgent:
    """
    Agent class that implements Dueling Architecture + Double DQN (D3QN).
    """
    team_id = "DuelingDDQNAgent"
    ...
    def __init__(self, agent_id: int, input_spec, num_actions: int, lr: float=1e-3, device: str="cpu", pretrained_model=None):
        ...
        if pretrained_model:
            ...
            self.q_net = DuelingDQNModel(self.map_shape, self.aux_dim, num_actions).to(device)
            self.optimizer = optim.Adam(self.q_net.parameters(), lr=self.lr, eps=1e-08, weight_decay=1e-5)

        self.target_net = DuelingDQNModel(self.map_shape, self.aux_dim, num_actions).to(device)
    ...
    def load_agent(self, pretrained_model):
        ...
        self.q_net = DuelingDQNModel(self.map_shape, self.aux_dim, self.num_actions).to(self.device)
...
def train_dueling_ddqn(user_id=0, enemy_type="simple", num_episodes=100, max_steps=500, seed=86, save_model=True, pretrained_model=None):
    import sys as _sys
    ...
    user_agent = DuelingDDQNTrainingAgent(user_id, input_spec, num_actions, lr=lr, device="cuda" if torch.cuda.is_available() else "cpu", pretrained_model=pretrained_model)
    ...
    reward_history = []
    win_history = []
    with tqdm(total=num_episodes, desc="Training Dueling DDQN") as pbar:
            ...
            for _ in range(max_steps):
                ...
                if len(buffer) >= batch_size:
                    ...
                    if save_model and user_agent.global_step % 500 == 0:
                        model_folder = f"ckpts/dueling_ddqn/dueling_ddqn_{enemy_type}_{num_episodes}_episodes_{max_steps}_steps_{seed}_seed"
    ...
    plot_loss(loss_history=loss_history, save_path=f"{model_folder}/dueling_ddqn_{enemy_type}_{num_episodes}_episodes_{max_steps}_steps_{seed}_seed_loss.png")
    plot_rewards(reward_history=reward_history, save_path=f"{model_folder}/dueling_ddqn_{enemy_type}_{num_episodes}_episodes_{max_steps}_steps_{seed}_seed_rewards.png")
    plot_win_rates(win_history=win_history, save_path=f"{model_folder}/dueling_ddqn_{enemy_type}_{num_episodes}_episodes_{max_steps}_steps_{seed}_seed_win_rates.png")
    plot_moving_average(data=reward_history, window_size=10, save_path=f"{model_folder}/dueling_ddqn_{enemy_type}_{num_episodes}_episodes_{max_steps}_steps_{seed}_seed_moving_average.png")
...
def training():
    ...
    if not args.skip_training:
        train_dueling_ddqn(enemy_type=args.enemy_type, 
...
class Agent:
    """Dueling Double DQN Agent for submission."""    
    ...
    def _load_checkpoint(self, checkpoint_path):
        try:
            ...
            self.q_net = DuelingDQNModel(self.map_shape, self.aux_dim, self.num_actions)
            self.q_net.load_state_dict(checkpoint["model_state_dict"])
