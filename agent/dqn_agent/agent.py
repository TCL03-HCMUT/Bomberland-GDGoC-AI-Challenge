    ...
    with tqdm(total=num_episodes, desc="Training DQN") as pbar:
        for ep in range(num_episodes):
            ...
            if save_model and (ep + 1) % 200 == 0:
                model_folder = f"ckpts/dqn/dqn_{enemy_type}_{num_episodes}_episodes_{max_steps}_steps_{seed}_seed"
...
class Agent:
    ...
    def _load_best_checkpoint(self):
        ...
        local_ckpts_dir = Path(__file__).parent / "ckpts" / "dqn"
        ...
        if not checkpoints:
            root_ckpts_dir = Path(__file__).resolve().parent.parent.parent / "ckpts" / "dqn"
        ...
        if checkpoints:
            ...
            if fallback.exists():
                ...
                print("[ERROR] DQNAgent found no checkpoints! Random actions will be used.")
